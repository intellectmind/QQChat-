...
def on_server_startup(server: PluginServerInterface):
    send_msg_to_all_groups(f"服务器 [{config.server_name}] 已运行")
...
def on_qq_command(
    ...
    # /tps 指令
        ...
        tps_command_handle(server, event, command, event_type)
    # /kill 指令
    elif command[0] == "kill":
        kill_command_handle(server, event, command, event_type)
    # /kick 指令
    elif command[0] == "kick":
        kick_command_handle(server, event, command, event_type)
...
def list_command_handle(server: PluginServerInterface, event: MessageEvent):
    # 使用函数属性来存储最后一次发送消息的时间戳
    if not hasattr(list_command_handle, "last_message_time"):
        list_command_handle.last_message_time = 0  # 初始化时间戳
    
    current_time = time.time()
    cooling_time = current_time - list_command_handle.last_message_time

    # 限制发送频率：30s
    if cooling_time < 30:
        cooling_time_countdown = 30 - round(cooling_time)
        message = f"{cooling_time_countdown}秒后再试试吧~"
    ...
    reply_with_server_name(event, message)
    # 更新时间戳
    list_command_handle.last_message_time = current_time
...
def bound_command_handle(server: PluginServerInterface, event: MessageEvent,
    ...
    if event_type in ADMIN_EVENT_TYPES:
        if len(command) == 1:
            ...
            if qq_number in data:
                # 解除对应的白名单
                cmd = f"!!wr remove {data[qq_number]}"
...
def bound_qq_to_player(server, event, player_name):
    ...
    elif player_name in data.values():
        qq_list = get_keys_by_value(data, player_name)
        qq = qq_list[0]
        reply_with_server_name(
            event,
            f"[CQ:at,qq={user_id}] {player_name} 已经被 {qq} 绑定~"
        )
    else:
...
def after_bound(server, event, user_id, player_name):
    if config.whitelist_add_with_bound:
        if config.whitelist_add_cmd_template is None or config.whitelist_add_cmd_template == '':
            server.execute(f"!!wr add {player_name}")
        else:
...
def kill_command_handle(
    ...
    if event_type not in [EventType.PRIVATE_NOT_ADMIN_CHAT]:
        user_id = str(event.user_id)
        if user_id in data.keys():
            cmd = f"kill {data[user_id]}"
            execute(server, event, cmd)
        else:
            reply_with_server_name(
                event,
                f"[CQ:at,qq={user_id}] 无法kill, 请通过/bound <Player>绑定游戏ID"
            )

def kick_command_handle(
        server: PluginServerInterface,
        event: MessageEvent,
        command: List[str],
        event_type: EventType
    ...
    if event_type not in [EventType.PRIVATE_NOT_ADMIN_CHAT]:
        user_id = str(event.user_id)
        if user_id in data.keys():
            cmd = f"kick {data[user_id]}"
            execute(server, event, cmd)
        else:
            reply_with_server_name(
                event,
                f"[CQ:at,qq={user_id}] 无法踢出, 请通过/bound <Player>绑定游戏ID"
            )
...
def tps_command_handle(server: PluginServerInterface, event: MessageEvent,
                           command: List[str], event_type: EventType):
    if event_type == EventType.NONE:
        return
    cmd = "forge tps"
